      ...
      <div className={`nb-drawer${drawerOpen ? ' open' : ''}`} aria-hidden={!drawerOpen}>
        ...
        <div className="nb-drawer-footer">
          {[
            ...
            { href: 'mailto:josephdehazounde@gmail.com', title: 'Email', d: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5E824B" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="15" rx="3" /><path d="M2 8l10 7 10-7" /></svg> },
        ...
        <div className="fc-mockups fc-entry">
          ...
          <div className="fc-mobile-wrap">
            ...
            <div className="fc-resp-badge">
              <AnimIcon type="check" size={12} color="#5E824B" /> 100% Responsive
            </div>
          </div>
...
function About() {
  ...
  return (
    <section id="about-section" className="sec" style={{ padding: 0, borderTop: '1px solid rgba(94, 130, 75,.08)' }}>
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(styleId)) {
      ...
      s.textContent = `
        ...
        .fm-item {
          position: relative;
          border-top: 1px solid rgba(94, 130, 75,.15);
          overflow: hidden;
        }
        .fm-item:last-child { border-bottom: 1px solid rgba(94, 130, 75,.15); }
        .fm-link {
          display: flex;
...
function SkillsSection() {
  ...
  return (
    <section id="skills-section" className="sec">
      ...
      <p style={{ fontFamily: "'Space Mono',monospace", fontSize: '.62rem', color: 'rgba(94, 130, 75,.55)', letterSpacing: '.2em', margin: '2.5rem 0 1rem' }}>
        // déplace ta souris pour explorer
      </p>
      <div style={{ position: 'relative', width: '100%', height: '520px', borderRadius: '16px', border: '1px solid rgba(94, 130, 75,.1)', overflow: 'hidden', background: 'rgba(94, 130, 75,.02)' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', gap: '1.2rem', padding: '2rem', pointerEvents: 'none', zIndex: 0 }}>
          {[...SKILLS.frontend, ...SKILLS.backend, ...SKILLS.tools].map(sk => (
            <span key={sk.name} style={{ fontFamily: "'Space Mono',monospace", fontSize: '.62rem', color: 'rgba(94, 130, 75,.18)', letterSpacing: '.15em', textTransform: 'uppercase' }}>
              {sk.name}
            </span>
          ))}
        </div>
        ...
        <div className="pricing-grid" key={currentTab} ref={scrollRef}>
          {tab.plans.map((p, i) => (
            <div key={i} className={`price-card gs-card${p.isPopular ? ' pop' : ''}`}>
              ...
              <ul className="price-feat">{p.features.map((f, j) => <li key={j}><AnimIcon type="check" size={11} color="#5E824B" /> {f}</li>)}</ul>
...
function GallerySection() {
  ...
  useEffect(() => {
    ...
    import('ogl').then(({ Camera, Mesh, Plane, Program, Renderer, Texture, Transform }) => {
      ...
      const mkFB = (glCtx, title) => {
        ...
        ctx.font = 'bold 52px Syne, sans-serif'; ctx.fillStyle = 'rgba(94, 130, 75,0.4)'
      ...
      const TECH_ICONS = {
        ...
        'Canvas API': { src: '/assets/icons/devicon/canvasapi/canvasapi-original.svg', color: '#96BD7D' },
  ...
  return (
    <section id="gallery-section" style={{ position: 'relative' }}>
      ...
      <div id="gl-detail-panel" style={{ position: 'absolute', bottom: '3rem', left: '50%', transform: 'translateX(-50%)', background: 'rgba(6,8,16,.88)', backdropFilter: 'blur(20px)', border: '1px solid rgba(94, 130, 75,.2)', borderRadius: '18px', padding: '1.4rem 2rem', width: 'min(92vw,540px)', opacity: 0, transition: 'opacity .35s ease,transform .35s ease', pointerEvents: 'none', zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem', alignItems: 'flex-end', flexShrink: 0 }}>
            ...
            <a id="gl-p-link" href="#" target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '.4rem', background: 'linear-gradient(135deg,#5E824B,#CC3300)', color: '#fff', padding: '8px 16px', borderRadius: '999px', fontFamily: "'Clash Display','Syne',sans-serif", fontWeight: 700, fontSize: '.68rem', letterSpacing: '.06em', textTransform: 'uppercase', textDecoration: 'none', pointerEvents: 'auto' }}>
              Voir →
            </a>
          </div>
        </div>
  ...
  const testimonialCards = TESTIMONIALS.map(t => (
    <div key={t.name} className="testi-card" style={{
      ...
      <p style={{
        ...
        <span style={{ fontSize: '2.2rem', lineHeight: .5, color: 'rgba(94, 130, 75,.3)', fontWeight: 800, display: 'block', marginBottom: '.4rem' }}>"</span>
        {t.text}
      </p>

      {/* Footer */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '.9rem', paddingTop: '1rem', borderTop: '1px solid rgba(94, 130, 75,.15)', marginTop: '1rem' }}>
        <div style={{
          width: 40, height: 40, borderRadius: '50%',
          background: 'linear-gradient(135deg,#5E824B,#CC3300)',
          ...
          <div style={{ marginTop: '1.5rem', display: 'flex', gap: '.5rem', flexWrap: 'wrap' }}>
            {TESTIMONIALS.map(t => (
              <span key={t.name} style={{
                ...
                color: 'var(--muted)', border: '1px solid rgba(94, 130, 75,.18)',
...
function ContactSection({ onToast }) {
  ...
  const nodeLinks = [
    { id: 'cojn-github', href: 'https://github.com/johaoooo', label: 'GitHub', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#5E824B" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22" /></svg> },
    ...
    <section id="contact">
      ...
      <div className="contact-grid">
        <div className="contact-info">
          ...
          {[
            { href: 'tel:+2290162108694', text: '+229 01 62 10 86 94', svg: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5E824B" strokeWidth="1.8"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.95 11.5a19.79 19.79 0 01-3.07-8.67A2 2 0 012.86 1h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L7.09 8.66a16 16 0 006.29 6.29l1.02-1.02a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" /></svg> },
            { href: 'mailto:josephdehazounde@gmail.com', text: 'josephdehazounde@gmail.com', svg: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5E824B" strokeWidth="1.8"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="M2 7l10 7 10-7" /></svg> },
          ].map((item, i) => (
          ...
          <div className="contact-item">
            <div className="c-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5E824B" strokeWidth="1.8"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" /><circle cx="12" cy="10" r="3" /></svg></div>
