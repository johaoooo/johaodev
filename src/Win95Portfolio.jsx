...
const BOOT_LINES = [
  { t: 300, cls: 'white', txt: 'JOHAO BIOS v3.0 — Copyright Joseph Dehazounde 2026' },
  { t: 600, cls: 'white', txt: 'CPU: Joseph D. CyberSec & Fullstack @ 3.6GHz' },
  { t: 900, cls: 'ok', txt: '[OK] RAM Check ............. 6 Projets vérifiés' },
  { t: 1100, cls: 'ok', txt: '[OK] Frontend drivers ....... React / Next.js ✓' },
  { t: 1300, cls: 'ok', txt: '[OK] Backend & Cyber ........ Django / Burp Suite ✓' },
  { t: 1500, cls: 'ok', txt: '[OK] Deployment module ....... Vercel / Render ✓' },
  { t: 1700, cls: 'ok', txt: '[OK] JohaoDev Lab ............ En ligne ✓' },
  { t: 1900, cls: 'warn', txt: '[WARN] Café restant ........... Critique' },
  { t: 2100, cls: 'ok', txt: '[OK] Windows 95 Wallpaper .... Chargé ✓' },
  { t: 2300, cls: 'info', txt: '[INFO] Localisation ........... Porto-Novo, BJ' },
  { t: 2600, cls: 'ok', txt: '[OK] Système prêt ............ JOHAOFOLIO OS' },
];
...
function useBootWebGL(canvasRef, active) {
  useEffect(() => {
    ...
    const chars = 'JOHAODEV01010CYBERSECURITY FULLSTACK REACT DJANGO BURPSUITE PYTHON WEBGL'.split('');
    let raf;
    const draw = () => {
  ...
  return (
    <div>
      ...
      <div className="flex-row gap-12 mb-12 w95-about-row" style={{ alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 4 }}>
            <a href={ME.site} target="_blank" rel="noreferrer"><button className="w95-btn primary" style={{ fontSize: 10 }}><Fa icon="globe" style={{ marginRight: 4 }} />{ME.agency}</button></a>
      ...
      <div className="w95-panel" style={{ marginBottom: 10 }}>
        ...
        <div style={{ fontSize: 11, lineHeight: 1.7, color: 'var(--text-main)', marginBottom: 6 }}>
          Spécialisé en <strong>Cybersécurité offensive (OWASP Top 10, Burp Suite, Kali Linux)</strong> et développement fullstack <strong>React, Next.js, Django REST et PostgreSQL</strong>.
        </div>
        <div style={{ fontSize: 11, lineHeight: 1.7, color: 'var(--text-main)', marginBottom: 8 }}>
          À travers <strong>JohaoDev</strong>, je conçois des applications web résilientes et sécurisées dès la conception (Secure by Design).
        </div>
        {/* Soft skills */}
      ...
      <div className="w95-panel">
        <div style={{ fontSize: 11, lineHeight: 2.2 }}>
          ...
          <div><Fa icon="shield-alt" style={{ marginRight: 6, color: 'var(--navy)', width: 14 }} />Audit OWASP & Pentest Web</div>
        </div>
      </div>
    ...
    <div>
      <div className="w95-panel mb-12">
        ...
        <div style={{ fontSize: 11, lineHeight: 2.2 }}>
          ...
          <div><Fa icon="shield-alt" style={{ width: 16, marginRight: 6, color: 'var(--navy)' }} />Audit OWASP & Pentest Web</div>
...
function WelcomeContent({ onOpen }) {
  const art =
    `╔═════════════════════════════════╗
║   JOHAO FOLIO — Win95/WinXP OS  ║
║   by Joseph DEHAZOUNDE          ║
║   JohaoDev · Porto-Novo, BJ     ║
╚═════════════════════════════════╝`;
  return (
    <div>
      <pre style={{
...
function StartMenu({ onOpen, onClose }) {
  const items = [
    ...
    { id: '_site', icon: 'globe', label: 'JohaoDev' },
  ];
  const handle = id => {
    onClose();
  ...
  return (
    <div className="w95-startmenu" onClick={e => e.stopPropagation()}>
      <div className="w95-sm-sidebar"><div className="w95-sm-sidetext">JohaoDev OS</div></div>
        ...
        >
          ...
          {openWindows.map(win => (
            <Window
              ...
              statusText={
                win.type === 'projects' ? `${PROJECTS.length} projets · JohaoDev Portfolio` :
